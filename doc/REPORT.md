...
* **Hybrid Data Ingestion:** Combining dynamic web scraping (for up-to-date web content) with static document loading (PDF presentation) to maximize context accuracy.
...
1.  **Ingestion:** The system loads data from `https://promtior.ai/` using `WebBaseLoader`. It conditionally checks for the existence of `Promtior_Presentation.pdf` to load it using `PyPDFLoader` if available.
...
* **Dependency Management on Windows:** * *Challenge:* Installing `chromadb` required C++ build tools not present in the environment.
